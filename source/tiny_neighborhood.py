...
def parse_target_args(host: str | None, port: int | None) -> tuple[str, int]:
    """Resolve target from CLI args, XBOX env, then config.ini."""
    resolved_host = (host or "").strip()
    resolved_port = port or DEFAULT_PORT

    env = os.environ.get("XBOX", "").strip()
    if not resolved_host and env:
        if ":" in env:
            h, p = env.rsplit(":", 1)
            resolved_host = h.strip()
            try:
                resolved_port = int(p.strip())
            except ValueError:
                raise SystemExit(f"Invalid XBOX port in {env!r}")
        else:
            resolved_host = env

    if not resolved_host and CONFIG_FILE.exists():
        cfg = configparser.ConfigParser()
        cfg.read(CONFIG_FILE)
        resolved_host = cfg.get("xbox", "host", fallback="").strip()
        if port is None:
            try:
                resolved_port = cfg.getint("xbox", "port", fallback=DEFAULT_PORT)
            except ValueError:
                resolved_port = DEFAULT_PORT

    if not resolved_host:
        raise SystemExit("No Xbox Debug IP set. Use --host HOST, XBOX=HOST[:PORT], or run: connect HOST")

    return resolved_host, resolved_port


def save_target_config(host: str, port: int):
    cfg = configparser.ConfigParser()
    cfg["xbox"] = {"host": host, "port": str(port)}
...
def print_entries(entries: list[dict]):
    rows = []
    for e in sorted(entries, key=lambda x: (not x.get("is_dir", False), x.get("name", "").lower())):
        typ = "<DIR>" if e.get("is_dir") else "FILE"
        name = e.get("name", "")
        size = "" if e.get("is_dir") else human_size(e.get("size", 0))
        modified = e.get("changed", "")
        rows.append((typ, name, size, modified))

    widths = [
        max([len("Type")] + [len(r[0]) for r in rows]) if rows else len("Type"),
        max([len("Name")] + [len(r[1]) for r in rows]) if rows else len("Name"),
        max([len("Size")] + [len(r[2]) for r in rows]) if rows else len("Size"),
        max([len("Modified")] + [len(r[3]) for r in rows]) if rows else len("Modified"),
    ]

    print(f"{'Type':<{widths[0]}}  {'Name':<{widths[1]}}  {'Size':>{widths[2]}}  {'Modified':<{widths[3]}}")
    print(f"{'-'*widths[0]}  {'-'*widths[1]}  {'-'*widths[2]}  {'-'*widths[3]}")
    for typ, name, size, modified in rows:
        print(f"{typ:<{widths[0]}}  {name:<{widths[1]}}  {size:>{widths[2]}}  {modified:<{widths[3]}}")


def cli_download_tree(client: XBDMClient, remote_dir: str, local_dir: str):
    ...
    for entry in client.dirlist(remote_dir):
        ...
        local_child = local_dir_path / name
        if entry.get("is_dir"):
            cli_download_tree(client, remote_child, str(local_child))
        else:
            client.get(remote_child, str(local_child))


def cli_upload_tree(client: XBDMClient, local_dir: str, remote_parent: str):
    local_dir_path = Path(local_dir)
    remote_root = rjoin(remote_parent, local_dir_path.name)
    client.mkdir_if_missing(remote_root)

    for dirpath, _, files in os.walk(local_dir_path):
        d = Path(dirpath)
        rel = d.relative_to(local_dir_path)
...
def cli_delete_tree(client: XBDMClient, remote_dir: str):
    for entry in client.dirlist(remote_dir):
        name = entry.get("name", "")
        if not name:
            continue
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="TinyNeighborhood",
        description="Tiny Neighborhood - OG Xbox XBDM GUI and CLI over TCP 731",
    )
    parser.add_argument("--host", help="Xbox Debug IP. Can also use XBOX=HOST[:PORT] or config.ini.")
    parser.add_argument("--port", type=int, help="XBDM TCP port. Default: 731.")
    parser.add_argument("--timeout", type=float, default=60.0)
    parser.add_argument("--gui", action="store_true", help="Launch GUI explicitly.")

    sub = parser.add_subparsers(dest="cmd")

    connect = sub.add_parser("connect", help="Test connection and save host/port to config.ini")
    connect.add_argument("connect_host", nargs="?", help="Xbox Debug IP to save. HOST:PORT is also accepted.")
    connect.add_argument("connect_port_pos", nargs="?", type=int, help="Optional port, e.g. connect 192.168.1.50 731")
    connect.add_argument("--connect-port", type=int, default=None, help="Port to save; default 731")
    ...
    rm.add_argument("--recursive", "-r", action="store_true")

    rmdir = sub.add_parser("rmdir")
    rmdir.add_argument("remote")
    rmdir.add_argument("--recursive", "-r", action="store_true")

    rename = sub.add_parser("rename")
    rename.add_argument("old")
    rename.add_argument("new")

    upload = sub.add_parser("upload", aliases=["put"], help="Upload a single file")
    upload.add_argument("local")
    upload.add_argument("remote")

    download = sub.add_parser("download", aliases=["get"], help="Download a single file")
    download.add_argument("remote")
    download.add_argument("local")

    upload_dir = sub.add_parser("upload-dir", aliases=["put-dir"], help="Upload a folder recursively")
    upload_dir.add_argument("local_dir")
    upload_dir.add_argument("remote_parent")

    download_dir = sub.add_parser("download-dir", aliases=["get-dir"], help="Download a folder recursively")
    download_dir.add_argument("remote_dir")
    download_dir.add_argument("local_parent")

    launch = sub.add_parser("launch", aliases=["run"])
    launch.add_argument("remote_xbe")
    launch.add_argument("--no-debug", action="store_true")
...
def require_gui_available() -> None:
    if TK_AVAILABLE:
        return

    print("Tiny Neighborhood GUI requires Tkinter, but Tk could not be loaded.", file=sys.stderr)
    print(f"Tkinter error: {TK_IMPORT_ERROR}", file=sys.stderr)
    print("", file=sys.stderr)
    print("CLI commands still work. Try:", file=sys.stderr)
    print("  TinyNeighborhood --help", file=sys.stderr)
    print("  TinyNeighborhood connect 192.168.1.50", file=sys.stderr)
    print("  TinyNeighborhood ls", file=sys.stderr)
    raise SystemExit(2)
...
def run_cli(argv: list[str]) -> int:
    ...
    if args.cmd == "connect":
        connect_host = (getattr(args, "connect_host", None) or args.host or "").strip()
        connect_port = args.connect_port or args.connect_port_pos or args.port or DEFAULT_PORT

        if connect_host and ":" in connect_host and args.connect_port is None and args.connect_port_pos is None and args.port is None:
            host_part, port_part = connect_host.rsplit(":", 1)
            connect_host = host_part.strip()
            try:
                connect_port = int(port_part.strip())
            except ValueError:
                raise SystemExit(f"Invalid port in {host_part!r}:{port_part!r}")

        if not connect_host:
            env = os.environ.get("XBOX", "").strip()
            if env:
                if ":" in env:
                    connect_host, env_port = env.rsplit(":", 1)
                    connect_host = connect_host.strip()
                    if args.connect_port is None and args.connect_port_pos is None and args.port is None:
                        try:
                            connect_port = int(env_port.strip())
                        except ValueError:
        ...
        if not connect_host:
            raise SystemExit("Usage: TinyNeighborhood connect HOST[:PORT] [PORT]")

        try:
            with XBDMClient(connect_host, connect_port, timeout=args.timeout) as c:
                drives = c.drivelist()
            save_target_config(connect_host, connect_port)
            print(f"Connected to {connect_host}:{connect_port}")
            print("Saved config.ini")
            print(f"Drives: {drives}")
            return 0
        except (OSError, XBDMError) as e:
            print(f"tiny-neighborhood: error: {e}", file=sys.stderr)
            return 1

    host, port = parse_target_args(args.host, args.port)

    try:
        with XBDMClient(host, port, timeout=args.timeout) as c:
            if args.cmd == "ping":
                ...
                if not args.command:
                    raise SystemExit("raw requires a command, e.g. raw drivelist")
                code, msg = c.cmd(" ".join(args.command))
                print(f"{code}- {msg}")
                if code == 202:
                    for line in c.multiline():
                        print(line)

            elif args.cmd == "ls":
                remote = xp(args.remote)
                if not remote:
                    print_entries(drive_entries_from_drivelist(c.drivelist()))
                else:
                    print_entries(c.dirlist(remote))

            elif args.cmd == "mkdir":
                c.mkdir(args.remote)
                print(f"Created {xp(args.remote)}")

            elif args.cmd == "rm":
                if args.dir or args.recursive:
                    if args.recursive:
                        cli_delete_tree(c, args.remote)
                    c.delete(args.remote, is_dir=True)
                else:
                    c.delete(args.remote, is_dir=False)
                print(f"Deleted {xp(args.remote)}")
                ...
                c.rename(args.old, args.new)
                print(f"Renamed {xp(args.old)} -> {xp(args.new)}")

            elif args.cmd in ("upload", "put"):
                remote = xp(args.remote)
                if args.remote.endswith(("\\", "/")):
                    remote = rjoin(remote, Path(args.local).name)
                c.put(args.local, remote)
                print(f"Uploaded {args.local} -> {remote}")

            elif args.cmd in ("download", "get"):
                remote = xp(args.remote)
                local = Path(args.local)
                if str(args.local).endswith(("/", "\\")) or local.is_dir():
                    local = local / remote_basename(remote)
                c.get(remote, str(local))
                print(f"Downloaded {remote} -> {local}")

            elif args.cmd in ("upload-dir", "put-dir"):
                cli_upload_tree(c, args.local_dir, args.remote_parent)
                print(f"Uploaded folder {args.local_dir} -> {rjoin(args.remote_parent, Path(args.local_dir).name)}")

            elif args.cmd in ("download-dir", "get-dir"):
                target = Path(args.local_parent) / remote_basename(args.remote_dir)
                cli_download_tree(c, args.remote_dir, str(target))
                print(f"Downloaded folder {xp(args.remote_dir)} -> {target}")

            elif args.cmd in ("launch", "run"):
                result = c.launch(args.remote_xbe, debug=not args.no_debug)
                print(f"Launched {xp(args.remote_xbe)}: {result}")
